...
// OLD OPPORTUNITY FORM HANDLER REMOVED
// Now using backend integration from admin.js (line 841+)
// This old handler was creating cards without Edit/Delete buttons
...
// ===== LOGIN ===== (Removed - using backend integration below)

// ===== SIGNUP ===== (Removed - using backend integration below)

// ===== FORGOT ===== (Removed - using backend integration below)
...
// Override the existing form handlers to use the backend API
...
document.getElementById('signupForm').addEventListener('submit', async function(e) {
    ...
    // Call backend API
    const result = await signupAPI(name, email, password, confirmPassword);
...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    if (result.success) {
        ...
        // Load opportunities after successful login
        setTimeout(async () => {
            showDashboard(email);
...
// Override the logout button
document.querySelector('.nav-item.logout').onclick = handleLogoutBackend;

// ===== FORGOT PASSWORD WITH BACKEND =====
...
document.getElementById('opportunityForm').addEventListener('submit', async function(e) {
    ...
    if (editId) {
        ...
        result = await updateOpportunityAPI(editId, opportunityData);
        ...
        result = await createOpportunityAPI(opportunityData);
    }
...
// ===== LOAD OPPORTUNITIES ON OPPORTUNITY TAB CLICK =====
document.querySelector('.nav-item[data-page="opportunity"]').addEventListener('click', async function() {
    await loadOpportunities();
});

// ===== AUTO-LOAD OPPORTUNITIES ON PAGE LOAD IF LOGGED IN =====
window.addEventListener('DOMContentLoaded', async function() {
    // Check if user is logged in by trying to get current admin
    const result = await getCurrentAdminAPI();
